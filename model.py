...
#         layers.append(L.Deconvolution2D(None, cngf, ksize=4, stride=1, pad=0, initialW=conv_init, nobias=True))
        layers.append(nn.ConvTranspose2d(3, cngf, kernel_size=4, stride=1, padding=0))
...
#             layers.append(L.Deconvolution2D(None, cngf // 2, ksize=4, stride=2, pad=1, initialW=conv_init, nobias=True))
            layers.append(nn.ConvTranspose2d(cngf // 2, cngf // 2, kernel_size=4, stride=2, padding=1))
            layers.append(nn.BatchNorm2d(cngf // 2, initial_gamma=bn_init))
            layers.append(nn.ReLU())
            cngf = cngf // 2
            csize = csize * 2
        layers.append(nn.ConvTranspose2d(cngf // 2, nc, kernel_size=4, stride=2, padding=1))
...
class DCGAN_D(nn.Module):
    def __init__(self, isize, ndf, nz=1, conv_init=None, bn_init=None):
        ...
        while csize > 4:
            in_feat = cndf
            out_feat = cndf * 2
            layers.append(nn.Conv2D(out_feat, out_feat, kernel_size=4, stride=2, padding=1))
...
class EncoderDecoder(nn.Module):
    def __init__(self, nef, ngf, nc, nBottleneck, image_size=64, conv_init=None, bn_init=None):
        super(EncoderDecoder, self).__init__(
            ...
            bn=nn.BatchNorm2d(nBottleneck, initial_gamma=bn_init),
            decoder=DCGAN_G(image_size, nc, ngf, conv_init, bn_init)
        )

    def encode(self, x):
        h = self.encoder.encode(x)
        h = nn.LeakyReLU(self.bn(h))
